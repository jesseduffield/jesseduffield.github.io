...
By declaring a `car` variable we're shadowing the `car` type meaning we can't use re-use it to instantiate a second car. I find this annoying. Yes, you could (arguable _should_) give the variable a more meaningful name like `myCar` or something, but I don't find this easy to read:
